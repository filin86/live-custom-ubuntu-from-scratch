...
# Ожидаем:
#   BootOrder: XXXX,YYYY,...
#   BootXXXX* system0  HD(1,GPT,...,)/File(\EFI\BOOT\BOOTX64.EFI) ... rauc.slot=system0 root=PARTLABEL=rootfs_A ...
#   BootYYYY* system1  HD(2,GPT,...,)/File(\EFI\BOOT\BOOTX64.EFI) ... rauc.slot=system1 root=PARTLABEL=rootfs_B ...
```
...
`root=` или не поднял initramfs. Для EFI-stub загрузки нельзя использовать
`root=/dev/disk/by-partlabel/...`: на таком раннем этапе эти symlink'и ещё
недоступны без userspace/udev. Используйте `root=PARTLABEL=rootfs_A` или
`root=PARTLABEL=rootfs_B`.
...
2. Проверить текущие записи:
   ...
   efibootmgr -v
   ```
3. Пересоздать запись проблемного slot'а с `root=PARTLABEL=...`:
   ```bash
...
Installer и RAUC `system.conf` создают EFI entries с `root=PARTLABEL=...`, а
не с `/dev/disk/by-partlabel/...`. Это защищает первый boot и обновление через
`rauc install` даже если initramfs не был подхвачен.
...
## 8. Netplan просит systemd-networkd

### Симптомы

- `netplan apply` пишет ошибку про `systemd-networkd`.
- NetworkManager установлен, но интерфейс не поднимается.
- В `/etc/netplan` лежат несколько yaml-файлов после объектного startup-скрипта.

### Причина

В RAUC-образе сеть ведёт NetworkManager, а `systemd-networkd` отключён и
замаскирован, чтобы не было двух сетевых управляющих одновременно. Если
объектный скрипт копирует yaml без `renderer` или с `renderer: networkd`,
netplan пытается применить часть конфигурации через `systemd-networkd`.
...
for f in /etc/netplan/*.yaml /etc/netplan/*.yml; do
    [ -e "$f" ] || continue
    sed -i 's/renderer:[[:space:]]*networkd/renderer: NetworkManager/I' "$f"
    grep -q '^[[:space:]]*renderer:' "$f" \
        || sed -i '/^[[:space:]]*version:/a\  renderer: NetworkManager' "$f"
done
