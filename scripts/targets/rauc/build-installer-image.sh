...
# НЕ кладутся: installer извлекает их из bundle'а через `rauc extract`
# после verify'а подписи, так что подпись защищает именно те байты,
# которые физически записываются на диск.
#
...
#   INSTALLER_KEYRING_SRC       путь к keyring.pem (по умолчанию pki/dev-keyring.pem)
...
# Installer сначала verify'ит подпись через keyring, затем `rauc extract`-ит
# efi.vfat и rootfs.img из bundle'а на целевой панели. Никаких отдельных
# raw-образов в payload'е не кладём — это устранило бы криптографическую
# связь между подписью и тем, что реально попадёт на disk.
cp "$BUNDLE_SRC" "$PAYLOAD_DIR/bundle.raucb"

# Keyring — обязателен: без него installer откажется extract'ить bundle.
KEYRING_SRC="${INSTALLER_KEYRING_SRC:-$REPO_ROOT/pki/dev-keyring.pem}"
...
Как использовать (до выхода полноценного bootable USB-ISO):
  1. Загрузите панель с любого Ubuntu/Debian Live USB (UEFI).
  2. Откройте terminal, станьте root (sudo -i).
  3. Скопируйте этот payload на live-систему и распакуйте:
       mkdir -p /opt/inauto-installer
       tar -I zstd -xf <payload>.tar.zst -C /opt
  4. Запустите installer:
       /opt/inauto-installer/install-to-disk.sh
  5. По окончании панель автоматически перезагрузится.
...
Автоматический backup + restore /home/inauto:
  ...
  non-removable устройствах (исключая TARGET_DEVICE), архивирует его
